...
Below is an example interaction with the Financial Advisor (`adk run financial_advisor`
or `adk web`, ADK-local flow). The exact output of the agent may be different
every time — this is one sampled run through all four steps (data analysis,
trading strategy, execution plan, risk evaluation) for ticker `AAPL`.
